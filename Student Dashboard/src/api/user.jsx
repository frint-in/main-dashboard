...
export async function addInternship() {
    const response = await axios.post(`${AXIOS_API_BASE_URL}/api/internship/addinternship`);
...
export async function findIntershipById(id) {
  try {
    console.log("hi1");
    const response = await axios.get(`${AXIOS_API_BASE_URL}/api/internship/find/${id}`);
    // const response = await axios.get(`${import.meta.env.VITE_REACT_API_BASE_ROUTE}/api/internship/all`);
    console.log("hi1");
